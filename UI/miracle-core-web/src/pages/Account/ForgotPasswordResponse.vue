      ...
      <v-card class="mx-auto pa-10" outlined elevation="5">
        ...
        <v-form>
          ...
          >
            <v-text-field
              ref="password"
              v-model="forgotResponseModel.password"
          ...
          >
            <v-text-field
              ref="password"
              v-model="forgotResponseModel.repassword"
              :error-messages="errors"
          ...
          <router-link id="router" tag="a" to="/account/login">
...
export default {
  ...
  methods: {
    async send() {
      ...
      if (result) {
        let response = await new AccountService().forgotPasswordResponse(this.forgotResponseModel);
  ...
  beforeMount() {
    this.forgotResponseModel.code = this.$route.params.code;
  },
};
</script>
