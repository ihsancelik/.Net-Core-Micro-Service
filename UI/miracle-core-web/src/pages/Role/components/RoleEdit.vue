<template>
  <ValidationObserver ref="observer">
    <v-card class="pa-10 pt-5" elevation="5">
      ...
      <h2 class="text-md-center">{{ translator("editRole") }}</h2>
      <hr class="mb-5" />
      ...
      <v-form>
        ...
        <hr class="mb-3" />
        <v-btn @click.prevent="roleEdit" color="primary" dark class="v-btn--block v-size--large mb-2">
          {{ translator("update") }}
        </v-btn>
      </v-form>
    </v-card>
...
export default {
  ...
  methods: {
    async initialize() {
      let response = await new RoleService().getById(this.roleId);
      if (response === ReturnConstraints.REFRESH) await this.initialize();

      this.roleModel = response.data;
    },
    async roleEdit() {
      let result = await this.$refs.observer.validate();
      if (result) {
        let response = await new RoleService().edit(this.roleModel, this.roleId);
