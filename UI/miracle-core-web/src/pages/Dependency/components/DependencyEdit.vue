<template>
  <ValidationObserver ref="observer">
    <v-card class="pa-10 pt-5" elevation="5">
      ...
      <h2 class="text-md-center">{{ translator("editDependency") }}</h2>
      <hr class="mb-3" />
      ...
      <v-form>
        ...
        <hr class="mb-2" />
        <v-btn @click.prevent="dependencyEdit" color="primary" dark class="v-btn--block v-size--large mb-2">
          {{ translator("update") }}
        </v-btn>
      </v-form>
    </v-card>
...
export default {
  ...
  methods: {
    async initialize() {
      let response = await new DependencyService().getById(this.dependencyId);
      if (response === ReturnConstraints.REFRESH) await this.initialize();
      this.dependencyModel = response.data;
    },
    async dependencyEdit() {
      ...
      if (result) {
        ...
        let response = await new DependencyService().edit(formData, this.dependencyId);
  ...
  created() {
    this.dependencyId = this.dependencyIdProp;
    this.initialize();
  },
};
</script>
