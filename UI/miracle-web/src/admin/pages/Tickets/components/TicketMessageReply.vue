    ...
    <v-card class="pa-10 pt-5" elevation="5">
      ...
      <v-form>
        ...
        <v-card-actions class="ml-5 mr-5 mt-5">
          <v-btn large outlined color="black" width="100%" @click.prevent="replyMessage">
            {{ translator("send") }}
          </v-btn>
        </v-card-actions>
      </v-form>
  ...
  methods: {
    async initialize() {
      ...
      if (productResponse) {
        ...
        this.selectedTitle = productResponse.data.title;
      }
    },
    async replyMessage() {
      ...
      if (result) {
        ...
        let ticketResponse = await new TicketService().create(formData);
        if (ticketResponse.success) this.close("closed", true);
        else {
          this.model = {};
          this.selectedComponent = MessageBox;
          this.componentProps = {
            messageTitleProp: this.translator("failed"),
            messageTextProp: ticketResponse.message,
          };
